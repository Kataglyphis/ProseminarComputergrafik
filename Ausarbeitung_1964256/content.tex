...
\chapter{Moderne Rendering-Pipeline}
    ...
    \label{sec:Geometry Stage}
        ...
        \label{subsec:Geometry Shader}
            ...
            einkommenden Daten(Position, Farbe,..) sowie das Kopieren. Das \textit{instancing} kann auch hier auf einem beliebigen Objekt mehrfach angewandgt
            werden.
...
\chapter{Unterschied moderne und klassische Rendering-Pipeline}
    ...
    viele Freiheiten in der Entwicklung geben. Frühere Konzepte wie z.B die OpenGL-States, welche die reine Auswahl von vorgegebene Zuständen gaben,
    ...
    nicht mehr gerecht. So waren in früher Versionen keine Beleuchtung pro Pixel(Phong Shading) möglich. Mehr Flexibilität auf Hardwareseite ermöglicht bessere 
    Anpassungsfähigkeit an verschiedenen zu rendernde Szenenzuständen. Mit dem \textit{unified shader} Ansatz gibt es keine Hardwareunterschiede zwischen 
    Vertex- und Fragment Shader mehr. Flexibles Vergrößern/Verkleinern der Shaderpools ermöglicht ein Reaktion auf Geomtrie(viele Eckpunkte) lastige Szenen. 
    Neue frei-programmierbare Shadereinheiten waren auch Teil der Anwort auf den Wunsch nach mehr Freiheit und Flexibilität. So wurden der Tessellation und Geometry 
    ...
    Einheiten eine Auslagerung von Arbeit auf die GPU. Sehr aktuelle Entwicklungen~\ref{sec:Meshshader} zeigen eine erneute Steigerung der freien Konfiguerierbarkeit.
