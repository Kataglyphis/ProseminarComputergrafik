...
assoziierten Eckpunkte, Normalen) den Model Transform an und gehen somit in den World Space. Dies geschieht meist in der Anwendung bevor die Geometrie
...
Nach dem Beschneiden liegen nur noch die sichtbaren Primitive vor und gelangen zum Viewport Transform.
...
\chapter{Moderne Rendering-Pipeline}
    ...
    die Anzahl der Primitive für die Geometriestufe zu verringern.(Für Raytracing besonders interessant ~\ref{sec:Real-Time Raytracing}).
    ...
    \label{sec:Geometry Stage}
        ...
        \label{subsec:Primitive Assembly}
            ...
            zeichnen(Instancing). Die Struktur des Vertex Array Objekt ist eine Optimierung zu den älteren Index Buffern(siehe~\ref{ch:TraditionellvsModern}).
            Viele Primitive teilen sich Eckpunkte. Durch eine Indexierung spart man sich Speicherplatz.
        ...
        \label{subsec:Vertex Shader}
            ...
            Interessant zeigt sich, was direkt vor dem Aufruf des Vertex Shaders passiert: input/primitive assembling~\ref{subsec:Primitive Assembly}. Der Shader behandelt bzw. verändert oder ignoriert als freiprogrammierbare Stufe 
            ...
            Das Ergebnis kann zum Tessellation oder Geometry Shader geschickt werden, abgespeichert oder direkt rasterisiert werden und damit als Input für 
            den Pixel Shader dienen.\ref{sec:Pixel Shader}
        ...
        \label{subsec:Tessellation}
            ...
            Ist eine optionale teil-programmierbare Stufe zur Geometrievervielfachung, d.h. ein eingehendes Primitiv wird weiter in viele Dreiecke unterteilt. Der \textit{Hull Shader}
            ...
            Dabei lassen sich zwei jeweils benachbarte Patches kontrolliert unterteilen durch geteilte Kontrollpunkte. Dadurch ist sichergestellt, dass keine Verschattungsrtefakte
        ...
        \label{subsec:Geometry Shader}
            ...
            In der Praxis hat sich der \textit{Geometry Shader} als zu starr, ineffektiv erwiesen~\cite{taskshader}. Die Weiterentwicklung Mesh Shader \ref{sec:Meshshader}, 
        ...
        \end{center}
            ...
            Bei der orthographischen Projektion haben wir eine achsenorientierte, umschließende Box (AABB). Aufgespannt durch Punkt
            (l,b,n) und (r,t,f)(Diese Box umschließt die Szenengeometrie).
    
            
            Nehmen wir unser Sichtfeld als eine beschnittene Pyramide wahr, welche durch eine \textit{near} und \textit{far plane} durchzogen wird, so haben wir eine 
            ...
            \end{center}
            \textit{Near plane} wird durch beide Punkte (l,b,n) und (r,t,n) aufgespannt und f ist die z-Koordinate der \textit{far plane}.
        ...
        Clipping, View-Port-Transform! Nach dem Clipping befinden wir uns im Window Space.
    ...
    \label{sec:Pixel Shader}
        ...
        vom Programmierer weitere Berechnungen vorgenommen werden. Dazu zählt eine pro Pixel Beleuchtungsberechnung(Phong Shading). Will man Texturen verwenden, so passiert das 
        nun hier.
        ...
        Falls nicht passiert nichts und das nächste Primitiv bzw. Fragment wird betrachtet (Szene ohne semi-transparente Objekte!). Haben wir semitransparente Objekte, so 
        ...
        haben~\ref{subsec:Projektionstransformation}(Weglassen der z-Komponente).

    \section{Per Fragment Operations}
    \label{sec:Per Fragment Operations}

        Auf den durch Rasterisierung entstandenen Fragmenten können noch einige Operationen vorgenommen werden. Denn bevor die Fragmente in den Framebuffer geschrieben
        werden, können wir durch konfigurieren Operationen und Test darauf anwenden. Es kann innerhalb dieser Arbeit nicht in angebrachter Tiefe auf dieses Thema
        eingegangen werden. Aktuelle Rendering-Pipelines machen massiven Gebrauch davon. Deswegen sei an dieser Stelle auf weiterführende Literatur 
        verwiesen~\cite{akenine2018real} und beispielhaft zwei wichtige Vertreter vorgestellt.
        
        \subsection{Stencil/Scissor Test}

        Mit dem Stencil Test können wir schlussendlich entscheiden, ob ein Fragment gezeichnet werden soll oder nicht durch den Vergleich mit einem Referenzwert und 
        Stencil Buffer. Der Scissor Test dient dazu, Teile außerhalb eines Rechtecks zu entfernen. Dies kann dazu genutzt werden, nur Teile einen innerhalb eines
        ...
        in der die beiden Farben gemischt werden bestimmt eine Funktion z.B Multiplikativ, Additiv..Diese Operation gewinnt vorallem bei semi-transparenten Objekten 
        an Bedeutung. Für den einfachsten Fall beim Alpha-Blending~\cite{alphablending} gilt: $OutputColor = \alpha*IncomingColor + (1 - \alpha)*OutcomingColor$
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=\linewidth]{Bilder/Koordinatensysteme.pdf}
        \label{Koordinatensysteme}
        \caption{Koordinatensysteme und ihre Transformationen}
    \end{figure}
    Die obige Abbildung~\ref{Koordinatensysteme} zeigt uns nocheinmal im Schnelldurchlauf die Kordinatensysteme/-transformationen, welche in der Rendering-Pipeline 
    durchgangen werden. Im Vertex Shader~\ref{subsec:Vertex Shader} wechselt man für Berechnungen etc. vom Modelspace in den Worldspace bevor man die endgültige
    Vertexposition,-normale, Texturkoordinate mit einer ModelViewProjektion-Transformation direkt in Clipkoordinaten transformiert um diese dem 
    Pixel Shader~\ref{sec:Pixel Shader} als Input zu liefern. Dabei ist die Reihenfolge von Projektionstransformation, Clipping und anschließender Teilung durch
    w (Dehomogenisierung) besonders wichtig für korrekte perspektivische Interpolation! 
