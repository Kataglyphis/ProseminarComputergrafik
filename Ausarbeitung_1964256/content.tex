...
\chapter{Moderne Rendering-Pipeline}
    ...
    \label{sec:Geometry Stage}
        ...
        "Gouraud shading" Beleuchtung per Pixel.
...
enormen Anzahl von Dreiecken an ihre Grenzen. Die damalige Einführung des Tessellation~\ref{subsec:Tessellation}- und des Geometryshaders~\ref{Geometry Shader}
...
der Pipeline als zu star und unflexibel. Für starre Szenen wird hohe Bandbreite veranschlagt, da wir in unserem Index Buffer jedesmal nachschauen müssen.
...
man nun die Geometrie selbst in \textit{Meshlets} auf und speichert diese in einen Buffer, wobei jedes \textit{Meshlet} für sich auf einen VBO zeigt.
Dadurch lassen sich die Eckpunkte effizienter wiederverwenden!
...
So kann der Task Shader mit der Control Shader Einheit der bisherigen Tessellation verglichen werden. 
Jedoch arbeiten wir hier mit mehreren Threads und mit frei wählbaren In- und Output! Mehr Flexibilität ist die Devise.
...
\begin{figure}[H]
    ...
    \caption{Neue Form des Task-/Meshshaders\cite{taskshader}}
\end{figure}
