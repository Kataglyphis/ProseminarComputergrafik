...
so liegt das Objekt zuerst in Form von vielen Eckpunkten(Dreiecken) in der Anwendung vor. Dabei befinden wir uns im "Model Space".
...
assoziierten Eckpunkte, Normalen) den "Model Transform" an und gehen somit in den "World Space". \par
...
Nun sind wir an dem Punkt angelangt, an dem wir unsere transormierten, porjezierten, sichtbaren Eckpunkte mit ihren Shadinginformationen vorliegen haben
(Außerdem auch Tiefenwert!). Ziel der Rasterisierung ist es nun jeden einzelnen Bildschirmpixel mit diesen Informationen einzufärben.
